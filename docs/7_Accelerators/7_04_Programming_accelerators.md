...
There are several compilers in developmentwith varying levels of support for
...
MAD ROCm provides several libraries that mimic (subsets of) libraries in the CUDA ecosystem,
also easing porting from NVIDIA to AMD hardware.

Intel has adapted several of its CPU librararies to use GPU acceleration also in its oneAPI platform.
